...
import com.laiteam.echowall.ui.notification.all.AllSection

class NotificationTabFragment : DaggerBaseLithoFragment<Any?>(){
    companion object {
    ...
    override fun initData() {
        arguments?.getString(MY_TAB_NAME)?.let { tabName ->
            setComponentAsync(RecyclerCollectionComponent.create(c).disablePTR(true)
                            .section(AllSection.create(SectionContext(c)).size(arguments?.getString(MY_TAB_NAME)!!.length).build()).build())
        }
    }
}